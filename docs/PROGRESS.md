...
## Next up
**Phase 2 planning (breeding and hatching). Phase 1 is finished: it is complete, playable, retuned, and ships as a Windows `.exe` (version 0.1.5, which adds the per-type shiny hue of step 1.9h, on top of 0.1.4's dev-mode delete button from step 1.9g). Step 1.9f is done (2026-09-21): the Brambletrundle and Riftsneak sprites are in the game (twelve in all), Brambletrundle's Form 2 is renamed Briarburl with its descriptions matched to the approved art, and every exe build now empties `release/` first. Step 1.9d (five small UI changes after 1.9c) is done: glass panels, the Roster and Nexus merged into one page called Nexus, the milestone table, a Delete save section, and the 0.1.2 rebuild.**
...
- **Build from a clean tree.** `npm run electron:pack` builds whatever is in the working tree and does not run the tests; if the tree has anyone's uncommitted edits, make a `git worktree add ../<name> HEAD`, junction `node_modules` into it, test and pack there, and remove the junction (`cmd //c rmdir node_modules`) **before** removing the worktree. That is how 0.1.2 was built. **Packing empties `release/` first (1.9f), and from a worktree that is the worktree's `release/`, not the main one:** after the pack, run `node electron/clean-release.mjs` in the main tree to clear its `release/` too, then copy the finished exes (setup, portable, blockmap, `win-unpacked/`) from the worktree's `release/` into the main `release/`, so the folder the designer opens holds only the new version.
...
- [x] 1.9h Per-type shiny hue: Void shinys turn teal (270 degrees) instead of the global 150, which left them a muddy olive (designer's request, 2026-09-21). Presentation only: no sim change, no balance change, save version stays 2. Packed as 0.1.5. See the 1.9h entry below.
...
- **A clean `release/` on every build (designer's rule, step 1.9f).** `npm run electron:pack` is now `npm run build && node electron/clean-release.mjs && electron-builder --win`: after the build and before electron-builder, `electron/clean-release.mjs` deletes everything inside `release/` (old installers, portable exes, blockmaps, `win-unpacked/`, builder debug files), so the folder only ever holds the newest version and the old exes no longer pile up for the designer to delete by hand. It resolves the folder from the repo root and refuses unless that is exactly `<repo>/release` and a real folder (a junction or symlink there is refused; links inside it are removed as links, never followed). A locked file (a running copy of the game, antivirus, an Explorer window open in `release/`) is retried five times, a second apart, then the script prints "close the game and any Explorer window in release/, then run it again" and exits 1, which stops the `&&` chain, so electron-builder never starts on a half-cleaned folder. Tested in `test/electron.test.ts` against a throw-away folder. Run `node electron/clean-release.mjs` alone to clear `release/` without building.
...
**The `release/` folder is now 1.1 GB (1,160,297,758 bytes)**: four versions of the installer and portable exe (about 206 MB each) plus `win-unpacked` and the blockmaps. **Nothing old was deleted.** ~~For the designer to delete by hand: the `0.0.0` and `0.1.0` files (about 411 MB).~~ **Superseded in 1.9f: `npm run electron:pack` now empties `release/` before every build (see "Desktop packaging"), so nothing is left for anyone to delete by hand.** Remember the `0.0.0` and `0.1.0` exes read a version-2 save as too-new.
...
- **Shiny:** the same `filter: hue-rotate(var(--shiny-hue))` with the same tuned `tuning.ui.shinyHueDeg` (150), on the `<img>` or the emoji. `CreatureArt` sets `--shiny-hue` itself, so it is no longer in `cardStyle`. No separate shiny art. **Since 1.9h the turn is per type (Void is 270); 150 is the default for the rest.**
...
### 2026-09-21, step 1.9f: Brambletrundle and Riftsneak sprites, the Briarburl rename, and a clean release folder (designer's request)

Presentation and tooling only: no sim change, no balance change, **save version stays 2**. Four commits (a fifth, this record, follows them): (1) `80f0265` the six sprites, (2) `55b4ae9` the rename and descriptions, (3) `d76794b` the clean release folder, (4) `6ba5caf` version 0.1.3. The exe was built from a clean worktree of (4).

**1. Sprites.** `brambletrundle-f1/f2/f3.png` and `riftsneak-f1/f2/f3.png` were copied unchanged (byte-identical, checked with `cmp`) from `D:\AI\sprites\cutouts\` into `src/ui/assets/creatures/`. The 1.9e lookup needed no code change. Each is 512 x 512 RGBA with transparent corners (checked by decoding the PNGs: 47 to 62 percent of the pixels fully transparent, the creature fills 36 to 50 percent, the same range as the first six), 169 to 305 KB. `test/sprites.test.ts` had to change: it used Brambletrundle as its "no art yet, falls back to the emoji" example (three places), so those now use Mossgear, and the shipped-sprite list (`SHIPPED`) covers all four lines, twelve files. `docs/art-pipeline.md`'s status line now names the four wired lines (it still said nothing was wired).

**2. Rename and descriptions** (docs and JSON in one commit, because `test/content.test.ts` compares them). **"Briartread" is now "Briarburl"** in `docs/content-data.md` (table row) and `src/data/species.json`. This is the designer's explicit decision of 2026-09-21 and overrides the "never rename forms" rule for this one name; nothing else in the repo named it (`docs/art-prompts.md` already said Briarburl; the queue item in this file is history). Form 1's line is unchanged. Form 2 is now "bigger, sturdier mossy tumbleweed golem: a vine ball around a wooden core, with moss, small white flowers, bark arms and clawed feet" and Form 3 "ancient, hulking, gentle wood-and-vine giant with tree-trunk arms and legs and a small kind face". **The Form 2 emoji went from a tractor to a log (🪵)**; no test pins it (the emoji test only needs a real pictograph, no letters, and three distinct emoji per creature). It is only the fallback for a missing sprite. Not seen on screen: a log is Unicode 13, which Windows 11's emoji font has, but I did not look.

**Riftsneak description lines were left alone, as asked. Against the approved art:** Form 1 "shy shadowy biped peeking from portals" contradicts it in two words, "shadowy" (the art is a bright violet) and "peeking from portals" (there is no portal; it stands in the open holding a stone ring), and does not mention the ring; "biped" is fair. Form 2 "sleek cat-like shadow" is mostly right (a sleek, prowling cat) but "shadow" is loose (violet, starry, ring on the tail). Form 3 "elegant starry void panther" is right on "starry" and "panther", but the approved panther is armoured in stone rings and looks sturdy rather than elegant; not a contradiction, a gap.

**3. A clean `release/` on every build** (designer's rule). `electron/clean-release.mjs` runs inside `npm run electron:pack` after the build and before electron-builder; see "Desktop packaging" and the CLAUDE.md commands table. It refuses unless the folder is exactly `<repo>/release` and a real folder, removes links as links, retries a locked file five times a second apart, then exits 1 with "close the game and any Explorer window in release/, then run it again". Four tests in `test/electron.test.ts` (839 pass now, 835 before): it empties the folder and keeps it, it does not follow a link inside it, it refuses a `release/` that is a link out of the repo or a folder that is not the repo root, it retries and then stops on a lock (and recovers from one that clears), and the script order in `package.json`.

**4. Version and build.** `package.json` and `package-lock.json` are 0.1.3 (the lock file's own version had been left at 0.1.0 since that release; only its two project-version lines and `package.json`'s one changed). `npm test`: 839 pass. `npm run build`: clean. **Packed from a clean git worktree of the version-bump commit** (the main tree carries the art window's uncommitted docs; see "Next up"), following "Build from a clean tree". The pack ran the clean script (the worktree's `release/` was empty, so it printed "release/ is already empty"). I then ran `node electron/clean-release.mjs` in the main tree, which removed the old 0.1.2 files and `win-unpacked` (six items, 522 MB) from the real `release/`, and copied the finished output in; hashes match. The worktree and its `node_modules` junction were removed.

| File (in `release/`) | Size |
|---|---|
| `Aetherbound-Idle-0.1.3-setup.exe` | 101.2 MB (106,097,097 bytes) |
| `Aetherbound-Idle-0.1.3-portable.exe` | 100.9 MB (105,813,632 bytes) |
| `Aetherbound-Idle-0.1.3-setup.exe.blockmap` | 112 KB |
| `win-unpacked/` | 21 files |

Nothing else is in the folder (`builder-debug.yml` was not copied). **Packaged smoke tests: load, progress and single instance all pass on the worktree's `win-unpacked` (`npm run electron:smoke:packaged`, 26 s) and on the portable exe in the main `release/` (45 s).** All six new sprite files are inside the packaged `app.asar`.

**Not verified (the designer asked me not to open the app).** I did not look at any sprite in the running game, on a card, in a slot, shiny or not, so scale, the rim colours and the emoji fallback are unseen. Riftsneak on the dark plate: I could only judge the **cut-out composited onto the plate colour** (`--art-plate` `#090c10`), not the game. There Form 3 reads: a mid-violet body, grey stone armour and a white chest and tail tuft separate clearly, and the dark indigo outline merges into the plate (expected) but the fill does not. Mean contrast of Riftsneak's opaque pixels against the plate: Form 1 3.9:1, **Form 2 2.9:1**, Form 3 4.0:1, against 6.9:1 for Emberfang Form 3; about two thirds of Form 3's pixels are under 3:1 by luminance alone, which is the dim look the queue item predicted. **Form 2 is the dimmest of the three.** What I cannot judge: the card's rarity tint and glow behind it, the Form 1 scale (0.72 of the plate), and a shiny (the 150 degree hue turn moves violet to a different colour and might be darker or lighter). **I did not change the plate**; the darker-accent idea and the lighter Void plate stay open in the queue.

**For the designer.** Run `release\Aetherbound-Idle-0.1.3-portable.exe`, turn on **Settings -> Dev panel**, then **Dev -> Grant creature** and pick **Brambletrundle** and **Riftsneak**, each in **Form 1, 2 and 3**, with and without the **Shiny** box ticked, and then look at the **Nexus** page. The granted creatures stay in your real save (Dev -> Reset save removes them).

**Next session name:** `Aetherbound P2 · 2.0 Phase 2 plan (Opus)` (unchanged: Phase 2 planning, per "Next up"). If the sprites need changes first: `Aetherbound P1 · 1.9g Sprite feedback (Sonnet)`.
...
- **Action:** `actions.deleteCreature(id)`, through the same `dev()` wrapper as the grants: steps to now, commits, saves at once.
- **UI:** `DeleteCreature` in `RosterCard.tsx` (a sibling of the card's toggle button, never inside it), the inline SVG trash can, `.rcard-delete` in `theme.css`. A 36 px button, smaller than the usual 44 px so it does not cover the art; the card is `position: relative` for it. An opened card in dev mode keeps 56 px free on the right so a long name ("Brambletrundle ✨ Shiny" at 375 px) wraps instead of running under it.
- **Tests** (848 pass, 839 before; `test/dev.test.ts`): the sim (removes only that creature and changes nothing else, benches a working one and leaves a valid, savable state, leaves other creatures and their work alone, does not reuse the id or touch the RNG, refuses an unknown id) and the action (saves at once and the file no longer holds it, a working creature's slot is empty in the save too, a refusal changes nothing, it survives a reload). Breaking the unassign step makes 4 of them fail.
- **Checked in the running app** (dev server, the Browser pane's own throw-away save, no console error): 15 cards with 15 buttons; one click arms and nothing is deleted; a second click removes the card and the saved file drops from 15 to 14; deleting the creature that was working Woodcutting empties its slot, and the Current activity panel says nothing is working; with the Dev panel off there are 0 buttons and with it on there is one per card; at 375 px no horizontal scroll and no overlap. **Not checked:** keyboard use (Tab, Enter, Escape) and a screen reader, and the packaged exe.

### 2026-09-21, step 1.9h: a shiny hue per type, so Void shinys turn teal (designer's request)

Every shiny used to get the same CSS `hue-rotate` of `tuning.ui.shinyHueDeg` (150). That suits green (Sproutlet turns purple) and orange (Emberfang turns teal), but Void violet lands on yellow and comes out a muddy olive. The designer compared angles on the real Riftsneak sprites and chose teal for Void, a turn of 270 degrees (the same as -90; the schema needs a value strictly between 0 and 360). Presentation only: no sim change, no balance change, **save version stays 2**, the saved file's shape is unchanged. Two commits: the code and tests (`5c63052`), then this record and the version.

**Naming note.** The brief called this step 1.9g and the version 0.1.4, but the repo already had a 1.9g (the dev-mode delete button on the Nexus cards) that was packed as 0.1.4. So this is **1.9h and version 0.1.5**, rather than a second 0.1.4 with different contents. The delete button is recorded in the 1.9g entry above (with its tests and checks), so nothing was added to the 1.9f entry.

- **Data.** `types.json` may give a type an optional `shinyHueDeg` (`z.number().gt(0).lt(360).optional()` in `TypeSchema`, the same rule as the tuning value). **Void has 270; no other type has one**, so Verdant, Telluric, Pyric, Aqueous and Voltaic all still get `tuning.ui.shinyHueDeg` (150) and nothing about them changes.
- **Selector.** `shinyHueFor(typeIds)` in `state/selectors.ts`: the first-listed type's own `shinyHueDeg` if it has one, else the tuning value (an unknown type or an empty list also falls back). **A hybrid uses its first-listed type**, so Eclipseed (Void / Verdant) and every other Void-first hybrid gets the Void teal, and a hybrid that merely has Void second would not (none exists today: all five Void hybrids list Void first). `CreatureView.shinyHueDeg` carries the result (set for every creature, used only when it is a shiny); the exported `shinyHueDeg` constant stays as the default.
- **UI.** `CreatureArt` sets `--shiny-hue` from `view.shinyHueDeg` for both the sprite and the emoji fallback. It was the only user of the global constant. The CSS is unchanged (`filter: hue-rotate(var(--shiny-hue))`). No per-shiny art.
- **Docs.** `docs/plan.md` (the `types.json` example and the `ui.shinyHueDeg` note) now says the value is the default and per-type values override it. CLAUDE.md rule 4 only says shinies use "a hue shift applied at runtime", not that it is one global value, so it is unchanged.
- **Tests** (855 pass, 848 before; `npm run build` clean): `content.test.ts` (a per-type value of 0, 360, -30, 400 or the string '270' is rejected with the path `types.json[0].shinyHueDeg`; 1, 90 and 359.5 load; omitting it loads; Void resolves to 270 and no other type has one), `selectors.test.ts` (a Void species, a Void-first hybrid, a non-Void species and a hybrid whose first type is not Void, plus `shinyHueFor` called directly for the Void-second order, an unknown type and an empty list), `sprites.test.ts` (the existing shiny test now pins the resolved hue per creature instead of the global one, and a new case checks Riftsneak and Hushflutter, sprite and emoji, at 270 and Sproutlet, Emberfang and Ashwood at 150, and that a normal Void creature has no hue turn). Making `shinyHueFor` ignore the per-type value fails 4 of them.
- **Not looked at.** No screenshots were taken (the brief said not to unless a test fails), so the teal itself is unseen by me; 270 is the designer's chosen angle. Sproutlet's purple is unchanged by construction (same 150, same code path, and the sprites test pins it).

**Art and content queue.** The designer's hybrid-shiny idea is added to the queue below as an idea, not built.

**For the designer.** Run `release\Aetherbound-Idle-0.1.5-portable.exe`, turn on **Settings -> Dev panel**, then **Dev -> Grant creature**: pick **Riftsneak** (or any Void creature) with the **Shiny** box ticked, and compare it with a **Sproutlet** shiny (still purple). Then look at the **Nexus** page. The granted creatures stay in your real save (Dev -> Reset save removes them).

**Next session name:** `Aetherbound P2 · 2.0 Phase 2 plan (Opus)` (unchanged: Phase 2 planning, per "Next up"). If the sprites need more changes first: `Aetherbound P1 · 1.9i Sprite feedback (Sonnet)`.

## Manual checklist for 0.1.5 (designer)

Run `release\Aetherbound-Idle-0.1.5-portable.exe`. The 0.1.3 list below still applies, except that its item 4 (the clean folder) now reads: `release\` holds only the 0.1.5 files. These are what is new.

1. **Void shinys are teal:** Dev panel on, Dev -> Grant creature: Riftsneak, Form 1, 2 and 3, Shiny ticked. Is the teal readable and clearly not olive? Try a Void hybrid too (for example Eclipseed): it takes the Void teal.
2. **Other shinys did not move:** a Sproutlet shiny is still purple and an Emberfang shiny still teal-blue, exactly as in 0.1.4.
3. **The emoji fallback follows the same rule:** a Void creature with no sprite yet (Hushflutter, Netherpod) with Shiny ticked shows its emoji in the same teal turn.
4. Saves are still version 2; the 0.1.4 save carries over.
...
### Art and content queue (designer decisions and ideas, 2026-09-21; the Brambletrundle wiring, the Briarburl rename and the description reconcile were done in 1.9f; the darker-accent idea and the Void plate are still open)
Sprites are made outside the repo (see `docs/art-pipeline.md`; ready-to-paste prompts in `docs/art-prompts.md`). Step 1.9e showed the first six sprites (Sproutlet and Emberfang, forms 1 to 3) with the emoji as the fallback; step 1.9f added Brambletrundle and Riftsneak (twelve in all).
...
- ~~**Sprite wiring for Brambletrundle:**~~ **Done in 1.9f (Riftsneak came with it).** Its three approved cut-outs are in `D:\AI\sprites\cutouts\` (`brambletrundle-f1.png`, `-f2.png`, `-f3.png`) and are not in the repo yet. Drop them into `src/ui/assets/creatures/` (the 1.9e lookup needs no code change; check the filename test).
- ~~**Rename "Briartread" to "Briarburl" (designer decision).**~~ **Done in 1.9f.** The Brambletrundle Form 2 no longer has treads. Touches `docs/content-data.md`, `src/data/species.json` and anything else that names it; names are otherwise locked. Do it together with the description lines below.
- ~~**Reconcile the Brambletrundle description lines with the approved art**~~ **Done in 1.9f (Form 2 and Form 3 as below, Form 1 unchanged; the Form 2 emoji became a log).** (doc line and JSON `art` text in one commit, because `test/content.test.ts` compares them). Form 1 stays "chaotic rolling ball of thorny vines and glowing bolts" (say painted bolts if the word "glowing" is a problem). Form 2 was "tank-like creature on tracks of thorns"; the approved Form 2 is a bigger, sturdier tumbleweed golem (vine ball around a wooden core, moss, small white flowers, bark-wood arms, clawed feet). Form 3 was "massive affectionate tumbleweed of vines"; the approved Form 3 is an upright, hulking, gentle wood-and-vine giant with tree-trunk arms and legs and a small face.
- **Hybrid shinys that mix the two parents' colours, different parts of the creature in different hues (designer idea). NOT built.** Cheap version: two stacked copies of the sprite with each parent's hue and a CSS mask gradient between them, no extra assets. A per-part version needs a hand-made mask per creature and conflicts with rule 4's "no per-shiny art", so it is a designer decision. Needs its own session and comparison images. (Today a hybrid shiny takes its first-listed type's hue, see 1.9h.)
