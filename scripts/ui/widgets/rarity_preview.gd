...
static func open() -> void:
	...
	sc.custom_minimum_size = Vector2(1100, 330)
	v.add_child(sc)
	var fill := func(_x = null):
		...
		for shiny in [false, true]:
			for r in range(1, Data.rarities.size() + 1):
				var cell := UI.vbox(2)
				cell.add_child(CreaturePortrait.make(id, form.selected + 1, r, shiny, 96))
