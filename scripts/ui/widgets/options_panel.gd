...
func _rebuild() -> void:
	...
	match _current:
		"audio":
			...
			var kinds := UI.label("Sound effects to play", "H3")
			_page.add_child(UI.margin(kinds, 0, 10, 0, 0))
			for g: Array in Sfx.GROUPS:
				var c := _toggle(g[1], g[0])
				# switching a kind on plays one of its sounds, so you hear what you turned back on
				c.toggled.connect(func(on): if on: Sfx.play(g[2]))
...
func _toggle(title: String, key: String) -> ToggleSwitch:
