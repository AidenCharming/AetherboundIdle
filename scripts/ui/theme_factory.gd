...
static func box(bg: Color, radius := 17, border := 0, border_color := Palette.LINE, margin := 14, shadow := 0) -> StyleBoxFlat:
...
static func get_theme() -> Theme:
	...
	for v in [["Title", F_HEAD_BOLD, 48, Palette.TEXT], ["H1", F_HEAD, 34, Palette.TEXT], ["H2", F_HEAD, 25, Palette.TEXT],
			["H3", F_HEAD, 20, Palette.TEXT], ["Dim", F_BODY, 18, Palette.TEXT_DIM], ["Faint", F_BODY, 16, Palette.TEXT_FAINT],
			["Small", F_BODY_BOLD, 16, Palette.TEXT_DIM], ["Num", F_BODY_BOLD, 20, Palette.TEXT]]:
	...
	# ---------------- panels
	t.set_stylebox("panel", "PanelContainer", box(Palette.PANEL, 22, 1, Palette.LINE, 19, 17))
	t.set_type_variation("Glass", "PanelContainer")
	t.set_stylebox("panel", "Glass", box(Palette.PANEL, 24, 1, Palette.LINE, 22, 22))
	t.set_type_variation("Card", "PanelContainer")
	t.set_stylebox("panel", "Card", box(Palette.CARD, 17, 1, Palette.LINE, 14, 7))
	t.set_type_variation("CardFlat", "PanelContainer")
	t.set_stylebox("panel", "CardFlat", box(Color(1, 1, 1, 0.035), 14, 1, Palette.LINE, 12))
	t.set_type_variation("Inset", "PanelContainer")
	t.set_stylebox("panel", "Inset", box(Color(0, 0, 0.05, 0.35), 14, 0, Palette.LINE, 12))
	t.set_type_variation("Pill", "PanelContainer")
	t.set_stylebox("panel", "Pill", box(Color(0.05, 0.06, 0.15, 0.75), 119, 1, Palette.LINE, 12))
	t.set_type_variation("Rail", "PanelContainer")
	t.set_stylebox("panel", "Rail", box(Color(0.045, 0.05, 0.12, 0.88), 0, 0, Palette.LINE, 12))
	t.set_type_variation("Modal", "PanelContainer")
	t.set_stylebox("panel", "Modal", box(Palette.PANEL_SOLID, 26, 1, Palette.LINE_STRONG, 26, 36))

	# ---------------- buttons
	_button(t, "Button", Palette.CARD, Palette.CARD_HOVER, Color(0.1, 0.1, 0.24), Palette.LINE, Palette.TEXT, 14)
	_button(t, "Primary", Color("5a4de0"), Color("6d60f2"), Color("4a3fc4"), Color(0.6, 0.9, 1.0, 0.5), Color.WHITE, 14, Palette.AETHER)
	_button(t, "Gold", Color("e2a93b"), Color("f2bd52"), Color("c99230"), Color(1, 0.9, 0.6, 0.6), Palette.INK, 14, Palette.GOLD)
	_button(t, "Danger", Color("b8445f"), Color("cf5572"), Color("9c3650"), Color(1, 0.6, 0.7, 0.4), Color.WHITE, 14)
	_button(t, "Ghost", Color(1, 1, 1, 0.0), Color(1, 1, 1, 0.06), Color(1, 1, 1, 0.1), Color(1, 1, 1, 0), Palette.TEXT_DIM, 12)
	_button(t, "Nav", Color(1, 1, 1, 0.0), Color(1, 1, 1, 0.05), Color(1, 1, 1, 0.08), Color(1, 1, 1, 0), Palette.TEXT_DIM, 12)
	_button(t, "NavActive", Color(0.42, 0.36, 1.0, 0.22), Color(0.42, 0.36, 1.0, 0.28), Color(0.42, 0.36, 1.0, 0.3), Color(0.5, 0.9, 1.0, 0.35), Palette.TEXT, 12)
	_button(t, "Chip", Color(0.05, 0.06, 0.15, 0.7), Color(0.12, 0.13, 0.3, 0.9), Color(0.2, 0.2, 0.45), Palette.LINE, Palette.TEXT_DIM, 119)
	_button(t, "ChipOn", Color(0.42, 0.36, 1.0, 0.35), Color(0.42, 0.36, 1.0, 0.45), Color(0.42, 0.36, 1.0, 0.5), Color(0.5, 0.9, 1.0, 0.5), Palette.TEXT, 119)
	_button(t, "Tile", Palette.CARD, Palette.CARD_HOVER, Color(0.1, 0.1, 0.24), Palette.LINE, Palette.TEXT, 19)
	_button(t, "TileOn", Color(0.2, 0.18, 0.45, 0.95), Color(0.24, 0.22, 0.5, 0.95), Color(0.2, 0.18, 0.45), Palette.AETHER, Palette.TEXT, 19)
	for v in ["Nav", "NavActive"]:
		t.set_font(&"font", v, font(F_BODY_BOLD))
		t.set_font_size(&"font_size", v, 19)
	for v in ["Primary", "Gold", "Danger"]:
		t.set_font(&"font", v, font(F_BODY_BOLD))
	for v in ["Chip", "ChipOn"]:
		t.set_font_size(&"font_size", v, 17)
		t.set_font(&"font", v, font(F_BODY_BOLD))

	# ---------------- progress bars
	t.set_stylebox("background", "ProgressBar", box(Color(0, 0, 0.05, 0.5), 119, 0, Palette.LINE, 0))
	t.set_stylebox("fill", "ProgressBar", box(Palette.AETHER, 119, 0, Palette.LINE, 0))
	t.set_font_size("font_size", "ProgressBar", 14)

	# ---------------- inputs
	var le := box(Color(0, 0, 0.05, 0.45), 12, 1, Palette.LINE, 12)
	t.set_stylebox("normal", "LineEdit", le)
	t.set_stylebox("focus", "LineEdit", box(Color(0, 0, 0.05, 0.55), 12, 1, Palette.AETHER, 12))
	...
	t.set_stylebox("focus", "TextEdit", box(Color(0, 0, 0.05, 0.55), 12, 1, Palette.AETHER, 12))

	# sliders
	var track := box(Color(0, 0, 0.05, 0.55), 119, 0, Palette.LINE, 0)
	track.content_margin_top = 4
	track.content_margin_bottom = 4
	t.set_stylebox("slider", "HSlider", track)
	var fill := box(Palette.AETHER_DEEP, 119, 0, Palette.LINE, 0)
	fill.content_margin_top = 4
	fill.content_margin_bottom = 4
	t.set_stylebox("grabber_area", "HSlider", fill)
	t.set_stylebox("grabber_area_highlight", "HSlider", fill)
	var grab := _circle_texture(22, Palette.AETHER)
	t.set_icon("grabber", "HSlider", grab)
	t.set_icon("grabber_highlight", "HSlider", _circle_texture(24, Color.WHITE))

	# option buttons, popups, check buttons
	_button(t, "OptionButton", Palette.CARD, Palette.CARD_HOVER, Color(0.1, 0.1, 0.24), Palette.LINE, Palette.TEXT, 12)
	t.set_stylebox("panel", "PopupMenu", box(Palette.PANEL_SOLID, 14, 1, Palette.LINE_STRONG, 10, 12))
	t.set_stylebox("hover", "PopupMenu", box(Color(0.42, 0.36, 1.0, 0.3), 10, 0, Palette.LINE, 7))
	...
	t.set_constant("icon_max_width", "PopupMenu", 34)
	...
	var tip := box(Color(0.10, 0.11, 0.25, 0.97), 14, 1, Palette.LINE_STRONG, 17, 0)
	tip.border_width_top = 4
	tip.border_color = Color(Palette.AETHER, 0.55)
	tip.shadow_color = Color(0, 0, 0.04, 0.6)
	tip.shadow_size = 17
	tip.shadow_offset = Vector2(0, 6)
	tip.content_margin_top = 12
	tip.content_margin_bottom = 12
	...
	var sbar := box(Color(1, 1, 1, 0.03), 119, 0, Palette.LINE, 0)
	sbar.content_margin_left = 4
	sbar.content_margin_right = 4
	var grabber := box(Color(1, 1, 1, 0.14), 119, 0, Palette.LINE, 0)
	...
	t.set_stylebox("grabber_highlight", "VScrollBar", box(Color(1, 1, 1, 0.24), 119, 0, Palette.LINE, 0))
	t.set_stylebox("grabber_pressed", "VScrollBar", box(Palette.AETHER, 119, 0, Palette.LINE, 0))
...
static func _button(t: Theme, v: String, normal: Color, hover: Color, pressed: Color, border: Color, text: Color, radius: int, glow := Color(0, 0, 0, 0)) -> void:
	...
	var n := box(normal, radius, 1, border, 17)
	var h := box(hover, radius, 1, border.lightened(0.2), 17)
	var p := box(pressed, radius, 1, border, 17)
	var d := box(Color(normal, normal.a * 0.45), radius, 1, Color(border, border.a * 0.4), 17)
	var f := box(Color(0, 0, 0, 0), radius, 2, Palette.AETHER, 17)
	f.draw_center = false
	if glow.a > 0:
		for sb in [n, h]:
	...
	t.set_constant("h_separation", v, 10)
	t.set_constant("icon_max_width", v, 31)
